...
##A node that is the parent to the Player's [MeshInstance3D](s), [AnimationController], and [Skeleton3D]
...
func _process(delta) -> void:
	animate.update(input, camera_handler.aim, clamp(((rad_to_deg(pitch_pivot.rotation.x))/45), -1, 1), is_on_floor(), delta)
	animate.update_move_state(input, running)
	camera_handler.update(direction, delta)
...
func movement(delta) -> void:
	...
	if input:
		direction = twist_pivot.basis * input.normalized() #Makes forward the direction the camera is facing rather than an cardinal direction.
